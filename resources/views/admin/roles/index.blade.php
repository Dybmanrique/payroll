...
@section('content')
    <div class="card">
        ...
        <div class="card-header">
            @can('roles.create')
                <a href="{{ route('roles.create') }}" class="btn btn-primary text-uppercase font-weight-bold">Registrar
                    nuevo</a>
            @endcan
        </div>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                url: "{{ route('roles.get_permissions') }}",
                type: "GET",
                dataType: 'json',
                ...
                const buttonsTemplate = {
                    edit: `<a class="btn btn-primary btn-sm mr-2 font-weight-bold btn-edit" href="{{ route('roles.edit', ':id') }}"><i class="far fa-edit"></i> EDITAR</a>`,
                ...
                table = applyDataTable('table', `{{ route('roles.data') }}`, columnAttributes, columnDefs);
            });

            $(`#table tbody`).on('click', '.btn-delete', function() {
                let data = table.row($(this).parents('tr')).data();
                deleteElement("{{ route('roles.destroy') }}", "{{ csrf_token() }}", table, data)
            });
        });
    </script>
@stop
